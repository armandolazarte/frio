...
class PedidoVendedorController {
	...
	function panel($arg) {
    	...
    	$select = "pv.pedidovendedor_id AS PEDVENID, CONCAT(date_format(pv.fecha, '%d/%m/%Y'), ' ', LEFT(pv.hora,5)) AS FECHA, UPPER(cl.razon_social) AS CLIENTE, UPPER(cl.nombre_fantasia) AS FANTASIA, pv.subtotal AS SUBTOTAL, FORMAT(pv.importe_total, 2,'de_DE') AS IMPORTETOTAL, UPPER(CONCAT(ve.APELLIDO, ' ', ve.nombre)) AS VENDEDOR, CASE pv.estadopedido WHEN 1 THEN 'inline-block' WHEN 2 THEN 'none' WHEN 3 THEN 'none' END AS DSPBTN, CASE pv.estadopedido WHEN 1 THEN 'SOLICITADO' WHEN 2 THEN 'PROCESADO' WHEN 3 THEN 'CANCELADO' WHEN 4 THEN 'A PROCESAR' WHEN 5 THEN 'PENDIENTE IMPRIMIR' END AS LBLEST, CASE pv.estadopedido WHEN 1 THEN 'primary' WHEN 2 THEN 'success' WHEN 3 THEN 'danger' WHEN 4 THEN 'warning' WHEN 5 THEN 'danger' END AS CLAEST, LPAD(pv.pedidovendedor_id, 8, 0) AS NUMPED, cl.cliente_id AS CLIID, pv.egreso_id AS EGRID, pv.detalle AS DETALLE";
	...
	//PROCESO POR LOTE
	function prepara_lote_vendedor() {
		SessionHandler()->check_session();
		require_once 'core/helpers/file.php';
		//$vendedor_id = $arg;
		$vendedor_id = filter_input(INPUT_POST, 'vendedor');

		$select = "pv.pedidovendedor_id AS PEDVENID, CONCAT(date_format(pv.fecha, '%d/%m/%Y'), ' ', LEFT(pv.hora,5)) AS FECHA, UPPER(cl.razon_social) AS CLIENTE, pv.subtotal AS SUBTOTAL, FORMAT(pv.importe_total, 2,'de_DE') AS IMPORTETOTAL, CASE pv.estadopedido WHEN 1 THEN 'inline-block' WHEN 2 THEN 'none' WHEN 3 THEN 'none' WHEN 4 THEN 'inline-block' WHEN 5 THEN 'none' END AS BTNAPROC, ep.denominacion AS LBLEST, CASE pv.estadopedido WHEN 1 THEN 'primary' WHEN 4 THEN 'warning' WHEN 5 THEN 'success' END AS CLAEST, LPAD(pv.pedidovendedor_id, 8, 0) AS NUMPED, cl.cliente_id AS CLIID, CASE WHEN pv.estadopedido = 5 THEN 'inline-block' ELSE 'none' END AS BTNAIMPR, pv.egreso_id AS EGRID, pv.detalle AS DETALLE";
		$from = "pedidovendedor pv INNER JOIN cliente cl ON pv.cliente_id = cl.cliente_id INNER JOIN vendedor ve ON pv.vendedor_id = ve.vendedor_id INNER JOIN estadopedido ep ON pv.estadopedido = ep.estadopedido_id";
		$where = "pv.vendedor_id = {$vendedor_id} AND pv.estadopedido IN (1,4,5) ORDER BY cl.razon_social ASC";
		$pedidovendedor_collection = CollectorCondition()->get('PedidoVendedor', $where, 4, $from, $select);
	...
	function traer_pedidovendedor_procesolote_ajax($arg) {
		...
		$select = "c.cliente_id AS CLIID, CONCAT(c.codigo, ' - ', c.razon_social) AS RAZON_SOCIAL";
		$from = "cliente c";
	...
	function guardar_procesar_lote() {
		...
		$porcentaje_listaprecio = $cm->listaprecio->porcentaje;
		$flete_id = $cm->flete->flete_id;
		$condicioniva_id = $cm->condicioniva->condicioniva_id;
