...
class StockController {
	...
	function consultar_producto($arg) {
		...
		$select = "s.stock_id AS STOID, date_format(s.fecha, '%d/%m/%Y') AS FECMOD, s.fecha AS FECHA, s.hora AS HORA, CONCAT(s.cantidad_actual, pu.denominacion) AS CANT, s.concepto AS CONCEPTO, CONCAT(s.cantidad_movimiento, pu.denominacion) AS MOVIMIENTO, CASE s.egreso_id WHEN 0 THEN 'none' ELSE 'inline-block' END AS BTNEGRID, s.egreso_id AS EGREID";
	...
	function update_num_factura() {
		...
		$where = "s.fecha = e.fecha AND s.hora = e.hora AND e.egreso_id = ed.egreso_id AND e.egreso_id = eafip.egreso_id AND e.fecha BETWEEN '2021-01-01' AND '2022-09-07' AND e.tipofactura IN (1,3) AND eafip.tipofactura = tf.tipofactura_id";
		...
		$where = "s.fecha = e.fecha AND s.hora = e.hora AND e.tipofactura = tf.tipofactura_id AND e.fecha BETWEEN '2021-06-01' AND '2022-09-07' AND e.tipofactura IN (2)";
		*/
