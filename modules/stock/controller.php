...
class StockController {
	...
	function consultar_producto($arg) {
		...
		$select = "s.stock_id AS ID, date_format(s.fecha, '%d/%m/%Y') AS FECMOD, s.fecha AS FECHA, s.hora AS HORA, CONCAT(s.cantidad_actual, pu.denominacion) AS CANT, s.concepto AS CONCEPTO, CONCAT(s.cantidad_movimiento, pu.denominacion) AS MOVIMIENTO, CASE s.egreso_id WHEN 0 THEN 'none' ELSE 'inline-block' END AS BTNEGRID, s.egreso_id AS EGREID";
